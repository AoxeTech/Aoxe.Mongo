...
namespace Zaabee.Extensions.Mongo.UnitTest
{
    public static class TestModelFactory
    {
        ...
        internal static List<KidTestModel> GetKidModels(int quantity, Guid parentId) =>
            Enumerable.Range(0, quantity).Select(p => GetKidTestModel(parentId)).ToList();

        internal static KidTestModel GetKidTestModel(Guid parentId)
        {
            return new KidTestModel
            {
                Id = _random.Next(),
                ParentId = parentId,
